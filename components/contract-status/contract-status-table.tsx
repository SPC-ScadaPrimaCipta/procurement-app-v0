...
export function ContractStatusTable() {
	...
	const handleMoveUp = (status: ContractStatus) => {
	...
	const handleMoveDown = (status: ContractStatus) => {
		const currentIndex = statuses.findIndex((s) => s.id === status.id);
	...
	const canMoveUp = (status: ContractStatus) => {
		return statuses.findIndex((s) => s.id === status.id) > 0;
	};

	const canMoveDown = (status: ContractStatus) => {
	...
	const handleSaveChanges = async () => {
		...
		try {
			...
			const response = await fetch("/api/master/contract-status/reorder", {
				method: "PUT",
