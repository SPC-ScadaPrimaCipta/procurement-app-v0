...
export function CaseStatusFormDialog({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			const url = status
				? `/api/master/case-status/${status.id}`
				: "/api/master/case-status";
			...
			toast.error(error.message || "Gagal menyimpan status pengadaan");
		} finally {
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-[500px]">
				<DialogHeader className="pr-8">
					<DialogTitle>
						{status ? "Edit Status Pengadaan" : "Tambah Status Pengadaan"}
					</DialogTitle>
				</DialogHeader>
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					<div className="space-y-2">
						...
						<Input
							...
							placeholder="Contoh: DRAFT, IN_PROGRESS, APPROVED"
							required
						/>
					</div>
