	...
	try {
		...
		if (!existing) {
			return NextResponse.json(
				{ error: "Status pengadaan tidak ditemukan" },
				{ status: 404 }
			);
		}
		...
		if (body.name && body.name !== existing.name) {
			const duplicate = await prisma.case_status.findFirst({
				where: {
					name: body.name,
					id: { not: id },
				},
		...
		const updated = await prisma.case_status.update({
			where: { id },
			data: {
				name: body.name,
...
export async function DELETE(
	...
	try {
		...
		await prisma.case_status.delete({
			where: { id },
		});
