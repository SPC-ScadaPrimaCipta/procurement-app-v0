...
export async function PUT(
	...
	try {
		...
		if (body.name && body.name !== existing.name) {
			const duplicate = await prisma.master_doc_type.findFirst({
				where: {
					name: body.name,
					id: { not: id },
				},
			...
			if (duplicate) {
				return NextResponse.json(
					{ error: `Jenis dokumen '${body.name}' sudah ada` },
					{ status: 409 }
				);
			}
		}

		const updated = await prisma.master_doc_type.update({
			where: { id },
			data: {
				name: body.name,
...
export async function DELETE(
	...
	try {
		...
		await prisma.master_doc_type.delete({
			where: { id },
		});
		...
		return NextResponse.json(
			{ error: "Failed to delete doc type" },
			{ status: 500 }
		);
	}
}
